    ...
    def write_data(self, data):
        try:
           ...
           """
            ...
            # 查看文件是否存在
            file_name = os.path.join(self.target_table_path, "SINO-" + str(self.filter_department)+"队领料单.xls")
            if os.path.exists(file_name):
                ...
                fin_data = []
                # 获取老数据
                for i in range(self.title_rowx + 1, rtb_nrows):
                    fin_data.append(rtb.row_values(i)[1:])
                # 删除过滤数据
                cp_data = deepcopy(fin_data)
                for i in cp_data:
                    if self.filter_date == i[8]:
                        fin_data.remove(i)

                fin_data.extend(data)
...
if __name__ == "__main__":
    ...
    try:
        for department in departments:
            ...
            data = eh.read_data()
            if len(data) == 0:
                print("部门%s: 今日[%s]无数据，不需要生成." % (department, filter_date ))
            else:
                eh.write_data(data)
                print("---   部门%s: 今日[%s]数据生成完成." % (department, filter_date))
    except Exception as e:
        print(str(e))
        print(traceback.format_exc())
